...
def _resolve_torch_device(device: str) -> Tuple[str, str]:
    ...
    if requested == "auto":
        if torch.backends.mps.is_available():
            return "mps", "torch_mps"
        return "cpu", "torch_cpu"
    if requested == "mps":
...
def _torch_pca_transform(
    ...
    q = min(max(int(n_components) + int(oversample), int(n_components)), min(X_centered.shape))
    U, S, V = torch.pca_lowrank(X_centered, q=q, center=False)

    k = int(n_components)
    scores = U[:, :k] * S[:k]
    total_var = torch.var(X_centered, dim=0, unbiased=True).sum()
    explained_var = (S[:k] ** 2) / max(1, X_centered.shape[0] - 1)
    explained_ratio_sum = float((explained_var.sum() / total_var).detach().cpu().numpy())

    X_pca = scores.detach().cpu().numpy().astype(np.float32, copy=False)
    return X_pca, explained_ratio_sum


def _sklearn_pca_transform(
    X: np.ndarray,
    n_components: int,
) -> Tuple[np.ndarray, float]:
...
def run_pca_hdbscan_sweep(
    ...
    valid_pc_list = []
    for n_pc in pc_list:
        if n_pc >= 1 and n_pc <= min(X.shape[0], X.shape[1]):
    ...
    print(f"[INFO] PCA backend: {pca_backend} (device={resolved_device})")
    ...
    for i, n_pc in enumerate(valid_pc_list, start=1):
        ...
        try:
            if pca_device.lower() == "sklearn":
                ...
                X_pca, explained = _torch_pca_transform(
                    X=X,
                    n_components=n_pc,
                    device=resolved_device,
                    oversample=torch_oversample,
                )
        except Exception as e:
            print(f"[WARN] Torch PCA failed for PCA={n_pc} on {resolved_device}: {e}")
        ...
        print(
            f"[INFO] Completed PCA {n_pc}: clusters={n_clusters}, "
            f"noise={noise_fraction:.2%}, explained={explained:.2%}, time={elapsed:.1f}s"
        )
        ...
        summary_rows.append(
            {
                ...
                "pca_device_requested": pca_device,
                "pca_device_used": resolved_device,
...
def main():
    parser = argparse.ArgumentParser(
        description="Run PCA at multiple dimensions on an NPZ array, cluster each PCA space with HDBSCAN, and plot labels on embedding_outputs. PCA can use torch on Apple MPS."
    )
    ...
    parser.add_argument("--torch-oversample", type=int, default=8)
    ...
    result = run_pca_hdbscan_sweep(
        ...
        torch_oversample=args.torch_oversample,
