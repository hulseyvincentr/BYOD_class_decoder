...
# -*- coding: utf-8 -*-
"""
cluster_sample_spectrograms_from_labels_with_subdirs.py

Generate stitched sample spectrograms for arbitrary cluster labels aligned to an
NPZ spectrogram time axis. Works with labels produced by the spectral-clustering
and PCA->HDBSCAN pipelines, including labels saved in:
...
This version can automatically save into a subdirectory under output_dir based
on the label-source filename, and can optionally prefix that subdirectory with a
user-supplied run name to keep parameter sweeps separated.
"""
...
def _sanitize_subdir_name(text: str) -> str:
    text = text.strip()
...
def _resolve_output_dir(
    ...
    use_label_source_subdir: bool = True,
) -> Optional[Path]:
    ...
    if run_name:
        parts.append(_sanitize_subdir_name(str(run_name)))

    if use_label_source_subdir:
        if labels_source_path is not None:
            parts.append(_sanitize_subdir_name(Path(labels_source_path).stem))
        else:
            parts.append(_sanitize_subdir_name(f"{Path(source_npz_path).stem}__{labels_key}"))

    outdir = base / "__".join(parts) if parts else base
    ...
    *,
    ...
    try:
        ...
        outdir = _resolve_output_dir(
            ...
            labels_key=labels_key,
            run_name=run_name,
            use_label_source_subdir=use_label_source_subdir,
        )
        if outdir is not None:
...
def main() -> None:
    ...
    parser.add_argument("--run-name", default=None, type=str, help="Optional prefix to distinguish parameter settings.")
    parser.add_argument("--no-label-source-subdir", action="store_true", help="Do not create a subdirectory from the label-source filename.")
    ...
    plot_spectrogram_samples_for_cluster_labels(
        ...
        use_label_source_subdir=not args.no_label_source_subdir,
    )
