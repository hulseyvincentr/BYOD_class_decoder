...
# -*- coding: utf-8 -*-
"""
spectral_cluster_npz_latent_space.py
...
Notes
-----
- Spectral clustering requires `n_clusters` up front.
- By default this script clusters `predictions`, which is often the higher-D
  latent representation. If you want to cluster a different array, set
  `array_key` accordingly.
- If `plot_key` exists and is 2D (for example `embedding_outputs`), the script
  will save a PNG colored by the new spectral labels.
...
- pandas
- matplotlib
- gpu_spectral_clustering installed so that `import gpu_spectral` works

CLI example
-----------
python spectral_cluster_npz_latent_space.py \
  ...
  --n-clusters 20 \
  --method auto \
  --out-dir /Volumes/my_own_SSD/updated_AreaX_outputs/spectral_outputs
"""
...
def plot_labels_2d(
    ...
    sc = ax.scatter(Xp[:, 0], Xp[:, 1], c=lp, s=3, alpha=0.7, linewidths=0)
    ax.set_xlabel("Dim 1")
    ax.set_ylabel("Dim 2")
    ax.set_title(title)
    cbar = fig.colorbar(sc, ax=ax)
    cbar.set_label("Spectral cluster")
    fig.tight_layout()
...
def main() -> None:
    ...
    cfg = SpectralConfig(
        ...
        plot_key=(None if args.plot_key.lower() == "none" else args.plot_key),
...
#
# import spectral_cluster_npz_latent_space as scn
...
# )
#
# master_csv = scn.run_root_directory_spectral(
#     root_dir=root_dir,
#     out_dir=root_dir / "spectral_cluster_outputs",
#     recursive=True,
#     cfg=cfg,
